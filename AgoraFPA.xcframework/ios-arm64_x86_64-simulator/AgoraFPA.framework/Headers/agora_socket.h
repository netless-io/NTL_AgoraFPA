...
typedef struct _agora_socket_event_handlers {
  ...
  /**
   * Triggered when connection state of fpa service changed
   * @param state Current connection state
   * @param reason Connection state change reason
   * @param user The user context
   */
  struct {
    void (*callback)(int state, int reason, void* user);
    void* opaque;
  } connection_state_changed;
  /**
   * Triggered when acceleration start
   * @param chain The chain id information
   * @param connection The connection id for this acceleration
   * @param user The user context
   */
  struct {
    void (*callback)(int chain, const char* connection, void* user);
    void* opaque;
  } acceleration_start;
  /**
   * Triggered when acceleration stop
   * @param chain The chain id information
   * @param connection The connection id for this acceleration
   * @param reason The acceleration stop reason
   * @param user The user context
   */
  struct {
    void (*callback)(int chain, const char* connection, int reason, void* user);
    void* opaque;
  } acceleration_stop;
